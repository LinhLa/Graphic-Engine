 ...
 */
void TaskDetruction::destruct(Task* pTask)
{
	_ASSERT(pTask);
	if (!pTask->is_done())
	{
		if (std::future_status::timeout == pTask->wait_for(300))
		{
			//<There is no way to terminate thread to stop from future
		}
	}
}
...
std::future_status Task::wait_for(uint32_t timeout)
{
	//blocking on result
	m_stop.store(true, std::memory_order_release);
	return m_f_result.wait_for(std::chrono::milliseconds(timeout));
}
...
void RepetitionTask::start()
{
	...
	auto main_loop = [&]() {
		...
		stream << std::hex << m_thread_id;
		LOG_DEBUG("Thread[0x%s] done", stream.str().c_str());
