...
#define USER_DEFINE_PROPERTY	"user_define_property"
...
#define PROPERTY_LIST 			"propertylist"
...
void SceneService::LoadResource()
{
	...
	picojson::array o_property_list = json_value.get(USER_DEFINE_PROPERTY).get<picojson::array>();
	for (picojson::array::iterator iter = o_property_list.begin(); iter != o_property_list.end(); ++iter)
	{
		...
		PropertyManager::add(name, value, upper, lower, std::stoi(type) & 0xFF);
	}
	...
	for (picojson::array::iterator iter = o_frames_list.begin(); iter != o_frames_list.end(); ++iter)
	{
		...
		for (picojson::array::iterator iter = o_property_list.begin(); iter != o_property_list.end(); ++iter)
		{
			...
			PropertyBase* pProperty = PropertyManager::create(name, value, upper, lower, std::stoi(type) & 0xFF);
			pObject->add(pProperty);
		}
