...
#define MAX_FREE_LIST_SIZE 			size_t(254)
...
protected:
	...
	static std::atomic_int64_t s_total;
	static std::atomic_int64_t s_usage;
public:
	static inline int64_t total() { return s_total.load(std::memory_order_relaxed); }
	static inline int64_t usage() { return s_usage.load(std::memory_order_relaxed); }
	static inline int64_t free() { return s_total.load(std::memory_order_relaxed) - s_usage.load(std::memory_order_relaxed); }
	IMemoryManager() {}
...
private:
	...
	FreeListObjectMemory<256> obj_memory_256;
