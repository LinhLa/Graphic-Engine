...
int64_t IMemoryManager::s_total = 0;
int64_t IMemoryManager::s_usage = 0;
...
void* FreeListObjectMemory<N>::allocate(const size_t& size)
{
	...
	if (!blockPtr)
	{
		...
		blockPtr = static_cast<uint8_t*>(base);
		base = &(static_cast<uint8_t*>(base)[N]);
	}

	_ASSERT(blockPtr);
	LOG_DEBUG("address[%08X] size[%d] guardbyte[%2X]", blockPtr, *(static_cast<uint8_t*>(blockPtr) - 1), static_cast<uint8_t*>(blockPtr)[N-2]);
	return blockPtr;
}
...
void FreeListMemoryManager::Initialise()
{
	...
	m_memory_list[6] = &obj_memory_255;
	...
	memset(&m_array_driven[127], 0x06, 253 - 126);
}
...
size_t DynamicMemorySizeManager::AssertGuardByte(void* object)
{
	...
	register uint16_t* pBlockSize = static_cast<uint16_t*>(object) - 1;
	register uint8_t*  pointer = static_cast<uint8_t*>(object);
...
void* DynamicMemorySizeManager::allocate(const size_t& size)
{
	...
	_ASSERT(blockPtr);
	LOG_DEBUG("address[%08X] size[%d] guardbyte[%2X]", blockPtr, *(static_cast<uint16_t*>(blockPtr) - 1), static_cast<uint8_t*>(blockPtr)[size]);
	return blockPtr;
}
...
void  DynamicMemorySizeManager::deallocate(void* object)
{
	...
	LOG_DEBUG("object[%X]\n", object);
	IMemoryManager::s_usage -= (2 /*Block size byte*/ + blockSize + 1 /*Guard Byte*/);
	uint8_t*  pointer = reinterpret_cast<uint8_t*>(object) - sizeof(uint16_t);
