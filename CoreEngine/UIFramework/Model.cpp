...
unsigned int TextureFromFile(const char* path, std::string directory)
{
    ...
    {
        LOG_DEBUG("Texture failed to load at path : %s", path);
        stbi_image_free(data);
    }
...
void Model::Draw(ShaderProgramPtr pShader)
{
...
MeshPtr Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
	...
	if(mesh->mMaterialIndex >= 0)
	{
		...
		std::vector<TextureRecord> diffuseMaps = loadMaterialTextures(material, aiTextureType_DIFFUSE, "texture_diffuse");
		textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
		std::vector<TextureRecord> specularMaps = loadMaterialTextures(material, aiTextureType_SPECULAR, "texture_specular");
		textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
	}  
	return Mesh::create(vertices, indices, textures);
}

MaterialPtr Model::processMaterial(aiMesh* mesh, const aiScene* scene)
{
...
std::vector<TextureRecord> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName)
{
	std::vector<TextureRecord> textures_loaded;
