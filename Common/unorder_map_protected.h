...
class unorder_map_protected
{
private:
	std::unordered_map<Key, T, Hash, KeyEqual, Allocator> m_internal_data;
	spinlock m_internal_lock;
	std::condition_variable_any m_condition_any;

public:
	using InternalType		= typename std::unordered_map<Key, T, std::less<Key>, Allocator>;
	...
	unorder_map_protected() {}
	unorder_map_protected(const unorder_map_protected& other)
	{
